...
public class LeftPanel extends JPanel { // drag source
	...
	public static JPanel getNewLabelFromText(String text) {
		...
		switch (text) {
		/*
		 * case Constants.LPAREN: newLabel = new DragAndDropLabel(new LParen());
		 * output1.setName("LParenOutput1");
		 * output1.setNextFocusableComponent(newLabel); newLabel.setOutputButton(0,
		 * output1); panel.add(newLabel); panel.add(output1); break; case
		 * Constants.RPAREN: newLabel = new DragAndDropLabel(new RParen());
		 * input1.setName("RParenInput1"); input1.setNextFocusableComponent(newLabel);
		 * newLabel.setInputButton(0, input1); panel.add(input1); panel.add(newLabel);
		 * break;
		 */
